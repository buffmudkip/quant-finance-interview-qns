...
| **4.3** | Conditional Probability & Bayes' Formula | Boys and girls (`1/3` vs `1/2`) · All-girl world? (`50%`) · Unfair coin (Bayes → `1024/2023`) · Fair bit from an unfair coin (von Neumann) · Dart game (→ `n/(n+1)`) · Russian roulette series (½ · go 2nd `5/11` · spin · don't spin) · Birthday line (→ 20th) · Dice order (`5/54`) · Monty Hall (switch → `2/3`) · Amoeba population (`√2−1`) · Candies in a jar (`7/12`) · Coin toss game (`4/9`) · Aces (`2197/20825`) · Gambler's ruin · Basketball scores (`1/99`) · Cars on road (`3/5`) |
| **4.4** | Discrete & Continuous Distributions | reference tables (CDF/pmf/pdf/E/Var) · discrete (Uniform, Binomial, Poisson, Geometric, Neg. Binomial) & continuous (Uniform, Normal, Exponential, Gamma, Beta) with use cases · Umamusume gacha (5 rate-up SSRs → ~667 pulls) · Meeting probability (`23/144`) · Probability of triangle (`1/4`) · Poisson waiting time (10 min; inspection paradox) · Moments of `N(0,1)` (`0,1,0,3`) |
| **4.5** | Expected Value, Variance & Covariance | concepts — covariance/correlation · variance & covariance algebra (bilinearity) · conditional expectation & variance · law of total expectation/variance (tower rule), each with a fair-die worked example · Connecting noodles (→ `∑1/(2k−1)`, ≈3.28) · Optimal hedge ratio (→ `ρσ_A/σ_B`) · Dice game (→ `7`) · Card game (first ace → `10.6`) · Sum of uniforms (→ `1/n!`) |
